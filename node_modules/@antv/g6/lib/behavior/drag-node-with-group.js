...
module.exports = {
  ...
  onDragStart: function onDragStart(e) {
    ...
    if (dragNodes.length === 0) {
      ...
      if (groupId) {
        ...
        var customGroup = customGroupControll.customGroup;
        var currentGroup = customGroup[groupId].nodeGroup;
        customGroupControll.setGroupStyle(currentGroup.get('keyShape'), 'hover'); // 初始拖动时候，如果是在当前群组中拖动，则赋值为当前groupId
   ...
   */
  dynamicChangeGroupSize: function dynamicChangeGroupSize(evt, currentGroup, keyShape) {
    var item = evt.item;
    ...
    if (nodes.length === 0) {
      ...
      var _customGroupControll$ = customGroupControll.calculationGroupPosition(nodes),
          x = _customGroupControll$.x,
          y = _customGroupControll$.y,
          width = _customGroupControll$.width,
          height = _customGroupControll$.height; // 检测操作的群组中是否包括子群组


      var paddingValue = customGroupControll.getGroupPadding(groupId);
  ...
  setCurrentGroupStyle: function setCurrentGroupStyle(evt) {
    ...
    if (this.inGroupId && groupId) {
      ...
      if (!(x < maxX * this.maxMultiple && x > minX * this.minMultiple && y < maxY * this.maxMultiple && y > minY * this.minMultiple) || this.inGroupId !== groupId) {
        ...
        this.dynamicChangeGroupSize(evt, currentGroup, keyShape); // 同时删除groupID中的节点
      ...
      this.dynamicChangeGroupSize(evt, _nodeInGroup, _keyShape);
    } else if (!this.inGroupId && groupId) {
      ...
      this.dynamicChangeGroupSize(evt, _currentGroup, _keyShape2);
      delete model.groupId;
    }
  ...
  _updateDelegate: function _updateDelegate(e, x, y) {
    ...
    if (!this.shape) {
      ...
      if (this.targets.length > 0) {
        ...
        var _customGroupControll$2 = customGroupControll.calculationGroupPosition(nodes),
            _x = _customGroupControll$2.x,
            _y = _customGroupControll$2.y,
