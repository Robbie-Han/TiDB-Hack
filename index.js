fetch('http://localhost:43000/graph').then(function(response) {
  ...
  /*node*/
  if(graphs.length !== 0) {
    let x = 100, y = window.innerHeight/2;
  ...
  if(heat.length !== 0) {
    ...
    for(let i = 0; i < data.nodes.length && heat.length > 0 ; i++){
      if(data.nodes[i].id !== heat[0].number + '') {
        data.nodes[i] = {...data.nodes[i], description: heat[0].sql, style: {lineWidth: 2, fill: 'rgb(' + heat[0].heat + ', 180, 180)'}};
        heat.shift()
      }
    }
  }
